...
class RepoApi:
    ...
    def upload_file(self, filepath, channel, package_type, name=None, version=None):
    ...
    def get_channel_artifacts_files(
        ...
        file_family_parsers = {
            ...
            "gra": self._parse_gra_file,
        }
        artifact_files = []
        total_count = 0
        data = self.get_channel_artifacts(channel).get("items", [])
        if package:
            # If package is provided without family, we fetch the family from package details
            packages = [
            ...
            if family:
                ...
                packages = [
                    {"name": pkg["name"], "family": pkg["family"]} for pkg in data
                ]
        ...
        for package in packages:
            if family:
                ...
                url = join(
                    self._get_channel_url(channel),
                    "artifacts",
                    package.get("family", family),
                    package.get("name", package),
                    "files",
                )
            resp = requests.get(url, headers=self.xauth_headers, verify=self.verify_ssl)
            files = self._manage_response(resp, "getting artifacts")
            ...
            for file_data in files["items"]:
                ...
                if rec:
                    ...
                    if "family" in file_data:
                        rec["family"] = file_data["family"]
    ...
    def _parse_gra_file(self, file_data, version, filename, return_raw=False):
        meta = file_data["metadata"]
        fn = file_data["ckey"].split("/", maxsplit=1)[-1]
    ...
    def get_artifacts(
        ...
        if artifact_family:
            url = f"{url}&artifact_family={artifact_family}"
        if platform:
    ...
    def get_user_id_channel_service_account(self, channel_name):
        url = self._get_channel_url(channel_name) + "/service-accounts"
        response = requests.get(url, headers=self.xauth_headers, verify=self.verify_ssl)
        return self._manage_response(response, "getting service account")

    def get_service_tokens_for_channel(self, channel_name, user_id):
        url = self._get_channel_url(channel_name) + "/service-accounts/%s/tokens" % (
            user_id
        )
        response = requests.get(
            url, headers=self.bearer_headers, verify=self.verify_ssl
    ...
    def create_channel_service_account_token(
        self,
        channel_name,
        user_id,
        token_name,
        ...
        if max_days:
            data["expires_at"] = str(
                datetime.datetime.now().date() + datetime.timedelta(days=max_days)
            )
        ...
        return self._manage_response(response, "creating service account token")

    def edit_channel_service_account_token(
        self, channel_name, user_id, token_id, token_name, max_days=None, resources=None
    ):
        url = self._get_channel_url(channel_name) + "/service-accounts/%s/tokens/%s" % (
            user_id,
            token_id,
        )
        ...
        return self._manage_response(response, "editing service account token")

    def delete_channel_service_account_token(self, channel_name, user_id, token_id):
        url = self._get_channel_url(channel_name) + "/service-accounts/%s/tokens/%s" % (
            user_id,
            token_id,
        )
        ...
        return self._manage_response(
            response, "deleting service account token", success_codes=[204]
        )

    def get_system_features(self):
        response = requests.get(self.urls["system"], headers=self.xauth_headers)

        if response.status_code == 200 and response.json().get("features"):
            return response.json().get("features")

        return self._manage_response(response, "getting system features")
